...
import zhuziJSON from "./zhuzi.json"
import zhuziMidJSON from "./zhuzi-mid.json"

const LanganUnitLength =1.7;
export const LanGan = async () => {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        matrix.setPosition(new THREE.Vector3(0, .1 * i, i * LanganUnitLength));
    ...
    */
    ...
    const zhuziMidLine = (from: THREE.Vector3, to: THREE.Vector3) => {
        ...
        const count = Math.floor(length / LanganUnitLength);
        const delta = new THREE.Vector3((to.x - from.x) / count, (to.y - from.y) / count, (to.z - from.z) / count);
        const scaleOffsetMatrix = new THREE.Matrix4().makeScale(1, 1, delta.length() / LanganUnitLength);
        for (let i = 0; i < count; i++) {
            ...
            const matrix = RotationMatrix.multiply(scaleOffsetMatrix);
            matrix.setPosition(pos);
    ...
    for (let i = 0; i < zhuziJSON.length; i++) {
        const data = zhuziJSON[i];
        for (let i = 0; i < data.length - 1; i++) {
            zhuziLine(new THREE.Vector3(data[i][0], data[i][1], data[i][2]), new THREE.Vector3(data[i + 1][0], data[i + 1][1], data[i + 1][2]),)
        }
    }

    for (let i = 0; i < zhuziMidJSON.length; i++) {
        const data = zhuziMidJSON[i];
        for (let i = 0; i < data.length - 1; i++) {
            zhuziMidLine(new THREE.Vector3(data[i][0], data[i][1], data[i][2]), new THREE.Vector3(data[i + 1][0], data[i + 1][1], data[i + 1][2]),)
        }
    }
    ...
    langan1Instance.castShadow = true;
    langan2Instance.castShadow = true;
    langan3Instance.castShadow = true;
    langan4Instance.castShadow = true;
    langan1Instance.receiveShadow = true;
    langan2Instance.receiveShadow = true;
    langan3Instance.receiveShadow = true;
    langan4Instance.receiveShadow = true;
